...
export function registerPrompts(server: McpServer, store: DataStore): void {
  server.prompt(
    "add_feature",
    "Guide for adding a new feature to the features tab with proper schema and context",
    { feature_description: z.string().describe("Brief description of the feature to add") },
    async ({ feature_description }) => {
      const featuresTab = TAB_CONFIGS.find((t) => t.key === "features")!;
      const existingFeatures = store.getTabData("features");
      return {
        messages: [{
          role: "user" as const,
          content: {
  ...
  server.prompt(
    ...
    {
      model_name: z.string().describe("Name of the data model entity (e.g., 'Order', 'Payment')"),
      description: z.string().optional().describe("Brief description of what this model represents"),
    },
    async ({ model_name, description }) => {
      const dataModelTab = TAB_CONFIGS.find((t) => t.key === "dataModel")!;
      const existingModels = store.getTabData("dataModel");
      return {
        messages: [{
          role: "user" as const,
          content: {
  ...
  server.prompt(
    ...
    {
      focus_area: z.string().optional().describe("Optional area to focus on (e.g., 'data model relationships', 'error coverage')"),
    },
    async ({ focus_area }) => {
  ...
  server.prompt(
    ...
    "Guide for adding a new screen to the Screen Map with proper relationships",
    {
      ...
      parent_screen: z.string().optional().describe("Parent screen ID (e.g., 'SCR-002')"),
    },
    async ({ screen_name, parent_screen }) => {
      const screensTab = TAB_CONFIGS.find((t) => t.key === "screens")!;
      const existingScreens = store.getTabData("screens");
      const existingFeatures = store.getTabData("features");
      return {
        messages: [{
          role: "user" as const,
